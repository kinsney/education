...
export function render(containerOrComponent, Component) 
{
    if (Component) 
    {
        return ReactDOM.render(Component, document.getElementById(containerOrComponent + '-mount'));
    } 
    else 
    {
        return ReactDOM.render(containerOrComponent, document.getElementById('test-mount')); }
}
...
export function contextClear(misago) {misago._context = {}; }
...
export function mockUser(overrides) 
{
    ...
    if (overrides) {return Object.assign(user, overrides); } 
    else {return user; }
}
...
export function snackbarStoreMock() 
{
    return {
        message: null,
        _callback: null,
        callback: function(callback) {this._callback = callback; },
        dispatch: function(action) 
        {
            if (action.type === 'SHOW_SNACKBAR') 
            {
                ...
                if (this._callback) 
                {
                    window.setTimeout(() => {this._callback(this.message); }, 100);
                }
            }
        }
    };
...
{
    if ($(selector).length) {ReactTestUtils.Simulate.click($(selector).get(0)); } 
    else {throw 'selector "' + selector + '" did not match anything'; }
}

export function simulateSubmit(selector) 
{
    if ($(selector).length) {ReactTestUtils.Simulate.submit($(selector).get(0)); } 
    else {throw 'selector "' + selector + '" did not match anything'; }
}
...
export function onElement(selector, callback) 
{
    let _getElement = function() 
    {
        window.setTimeout(function() 
        {
            ...
            if (element.length >= 1) {callback(element); } 
            else {_getElement(); }
        }, 50);
    };
