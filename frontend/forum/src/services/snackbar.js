...
export class Snackbar 
{
    ...
    info(message) {this.alert(message, 'info'); }

    success(message) {this.alert(message, 'success'); }

    warning(message) {this.alert(message, 'warning'); }

    error(message) {this.alert(message, 'error'); }
    ...
    apiError(rejection) 
    {
        ...
        if (!message) 
        {
            if (rejection.status === 404) { message = gettext("Action link is invalid."); } 
            else { message = gettext("Unknown error has occured."); }
        }
