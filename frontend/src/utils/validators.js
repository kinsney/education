...
export function required() 
{
    ...
    {
        if ($.trim(value).length === 0) {return gettext("This field is required."); }
    };
}
...
export function email(message) 
{
    return function(value) 
    {
        if (!EMAIL.test(value)) {return message || gettext("Enter a valid email address."); }
    };
}
...
export function minLength(limitValue, message) 
{
    return function(value) 
    {
        ...
        {
            if (message) {returnMessage = message(limitValue, length); } 
            else 
            {
